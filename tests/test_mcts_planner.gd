...
func test_mcts_planner_returns_action_sequence() -> String:
	...
	return run_checks([
		assert_true(sequence.size() > 0, "MCTS should return at least one action"),
		assert_true(sequence.size() > 1, "MCTS should usually plan more than just end_turn"),
	])
...
func test_mcts_planner_sequence_ends_with_end_turn_or_attack() -> String:
	...
	return run_checks([
		assert_true(
			...
			"Final action should be end_turn or attack, got %s" % last_kind
		),
	])
...
func test_mcts_planner_discovers_bench_before_attack() -> String:
	...
	return run_checks([
		assert_true(has_bench or has_attack, "Sequence should include either bench development or an attack"),
		assert_true(sequence.size() >= 2, "MCTS should plan a multi-step sequence"),
	])
...
func test_mcts_planner_does_not_modify_original() -> String:
	...
	return run_checks([
		assert_eq(gsm.game_state.players[0].hand.size(), original_hand_size, "MCTS should not mutate the original hand"),
		assert_eq(gsm.game_state.players[0].bench.size(), original_bench_size, "MCTS should not mutate the original bench"),
	])
