class_name ArceusGiratinaSelfPlayDataExporter
extends RefCounted

## 阿尔宙斯/骑拉帝纳卡组自博弈数据收集器。
## 使用 ArceusGiratinaStateEncoder 编码特征，额外记录 teacher_score（手写 evaluate_board 归一化值）。
##
## 训练策略：
## - 只保留赢家视角的记录（输家数据丢弃）
...
var base_dir: String = "user://training_data/arceus_giratina"
...
func record_state(game_state: GameState, current_player: int) -> void:
	var features: Array[float] = ArceusGiratinaStateEncoderScript.encode(game_state, current_player)
...
func export_game() -> String:
	...
	var data := {
		"version": "3.0",
		"encoder": "arceus_giratina",
		"feature_dim": ArceusGiratinaStateEncoderScript.FEATURE_DIM,
	...
	if file == null:
		push_warning("[ArceusGiratinaSelfPlayDataExporter] 无法写入: %s" % path)
		return ""
