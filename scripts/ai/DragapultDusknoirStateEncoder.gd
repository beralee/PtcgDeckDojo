class_name DragapultDusknoirStateEncoder
extends RefCounted

## 多龙巴鲁托/夜巡灵卡组专用 100 维特征编码器。
...
static func _encode_identity(player: PlayerState, features: Array[float], offset: int) -> void:
	...
	# [3] Duskull 数
	features[offset + 3] = clampf(float(_count_name_en(all_slots, DUSKULL)) / 2.0, 0.0, 1.0)
	# [4] Dusclops 数
	features[offset + 4] = clampf(float(_count_name_en(all_slots, DUSCLOPS)) / 1.0, 0.0, 1.0)
	# [5] Dusknoir 数
	features[offset + 5] = clampf(float(_count_name_en(all_slots, DUSKNOIR)) / 1.0, 0.0, 1.0)
	# [6] 准备好的攻击手数
	var ready: int = 0
	for slot: PokemonSlot in all_slots:
		if _can_attack(slot):
			var cd: CardData = slot.get_card_data()
			if cd != null and (cd.name_en == DRAGAPULT_EX or cd.name_en == DUSKNOIR):
				ready += 1
	...
	# [9] Dusknoir 体系已就绪 (Dusknoir >= 1)
	features[offset + 9] = 1.0 if _count_name_en(all_slots, DUSKNOIR) >= 1 else 0.0
...
static func _encode_resources(player: PlayerState, game_state: GameState, player_index: int, features: Array[float], offset: int) -> void:
	...
	# [6] 手牌有 Counter Catcher
	features[offset + 6] = 1.0 if _hand_has_en(player, COUNTER_CATCHER) else 0.0
	# [7] 手牌超能量数
	...
	features[offset + 8] = clampf(float(deck_psychic) / 15.0, 0.0, 1.0)
	# [9] 场上总能量数
	var field_energy: int = _count_field_energy(player)
	features[offset + 9] = clampf(float(field_energy) / 10.0, 0.0, 1.0)
	# [10] Dragapult ex 身上能量数
	var dragapult_energy: int = _count_energy_on_name(player, DRAGAPULT_EX)
	features[offset + 10] = clampf(float(dragapult_energy) / 4.0, 0.0, 1.0)
	# [11] 空板位数
	features[offset + 11] = clampf(float(maxi(0, 5 - player.bench.size())) / 5.0, 0.0, 1.0)
	# [12] 弃牌堆有进化棋子
	features[offset + 12] = 1.0 if _has_in_discard_names_en(game_state, player_index, [DRAKLOAK, DRAGAPULT_EX, DUSCLOPS, DUSKNOIR]) else 0.0
	# [13] 弃牌堆有 Dragapult ex
	features[offset + 13] = 1.0 if _has_in_discard_names_en(game_state, player_index, [DRAGAPULT_EX]) else 0.0
	# [14] 进化线缺口（场上 Dreepy/Drakloak 还需要多少才完成进化）
	var all_slots: Array[PokemonSlot] = _get_all_slots(player)
	var dreepy_count: int = _count_name_en(all_slots, DREEPY)
	var drakloak_count: int = _count_name_en(all_slots, DRAKLOAK)
	var evo_gap: int = maxi(0, dreepy_count + drakloak_count)  # 需要进化的基础/中间形态
	features[offset + 14] = clampf(float(evo_gap) / 3.0, 0.0, 1.0)
	# [15] 手牌有 Earthen Vessel
	features[offset + 15] = 1.0 if _hand_has_en(player, EARTHEN_VESSEL) else 0.0
...
static func _encode_attacker_detail(my_player: PlayerState, opp_player: PlayerState, features: Array[float], offset: int) -> void:
	...
	for opp_slot: PokemonSlot in opp_player.bench:
		...
		if opp_slot.get_remaining_hp() <= 60:
			# Phantom Dive 对后备放 6 个伤害指示物 = 60 伤害
	...
	# [6] Dusknoir 伤害指示物重分配总量（场上所有己方伤害指示物）
	var counter_total: int = 0
	for slot: PokemonSlot in all_slots:
		counter_total += slot.damage_counters
	features[offset + 6] = clampf(float(counter_total) / 300.0, 0.0, 1.0)
	# [7] 对手后备有低 HP 目标 (< 60)
