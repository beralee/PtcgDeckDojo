class_name DeckStrategyCharizardExBaseline
extends "res://scripts/ai/DeckStrategyBase.gd"


const STRATEGY_ID := "charizard_ex_baseline"
...
func _score_trainer(action: Dictionary, game_state: GameState, player_index: int) -> float:
	...
	match name:
		...
		BUDDY_BUDDY_POFFIN:
			if _count_live_poffin_shell_targets(player, game_state) == 0 and _should_defer_duskull_lane(player, game_state):
				return -40.0
...
func _score_arven(player: PlayerState, game_state: GameState, player_index: int) -> float:
	...
	if _deck_has(player, BUDDY_BUDDY_POFFIN) and combo_missing >= 2:
		item_value = maxf(item_value, 360.0)
	if _deck_has(player, NEST_BALL) and combo_missing >= 1 and _count_name(player, PIDGEOT_EX) == 0:
