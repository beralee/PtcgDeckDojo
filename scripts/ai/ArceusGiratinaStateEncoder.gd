class_name ArceusGiratinaStateEncoder
extends RefCounted

## 阿尔宙斯/骑拉帝纳卡组专用 100 维特征编码器。
...
static func encode(game_state: GameState, perspective_player: int) -> Array[float]:
	...
	# 攻击手细节: 84-91
	_encode_attacker_detail(my_player, opp_player, game_state, features, 84)
...
static func _encode_identity(player: PlayerState, features: Array[float], offset: int) -> void:
	...
	# [0] Arceus V 数
	features[offset + 0] = clampf(float(_count_name_en(all_slots, ARCEUS_V)) / 2.0, 0.0, 1.0)
	# [1] Arceus VSTAR 数
	features[offset + 1] = clampf(float(_count_name_en(all_slots, ARCEUS_VSTAR)) / 1.0, 0.0, 1.0)
	# [2] Giratina V 数
	features[offset + 2] = clampf(float(_count_name_en(all_slots, GIRATINA_V)) / 2.0, 0.0, 1.0)
	# [3] Giratina VSTAR 数
	features[offset + 3] = clampf(float(_count_name_en(all_slots, GIRATINA_VSTAR)) / 1.0, 0.0, 1.0)
	# [4] Bibarel 数
	features[offset + 4] = clampf(float(_count_name_en(all_slots, BIBAREL)) / 1.0, 0.0, 1.0)
	# [5] Bidoof 数
	features[offset + 5] = clampf(float(_count_name_en(all_slots, BIDOOF)) / 1.0, 0.0, 1.0)
	# [6] 准备好的攻击手数
	var ready: int = 0
	for slot: PokemonSlot in all_slots:
		if _can_attack(slot):
			var cd: CardData = slot.get_card_data()
			if cd != null and (cd.name_en == ARCEUS_VSTAR or cd.name_en == GIRATINA_VSTAR or cd.name_en == IRON_LEAVES_EX):
				ready += 1
	...
	# [8] 引擎已启动（Bibarel >= 1）
	features[offset + 8] = 1.0 if _count_name_en(all_slots, BIBAREL) >= 1 else 0.0
	# [9] Iron Leaves ex 在场
	features[offset + 9] = 1.0 if _count_name_en(all_slots, IRON_LEAVES_EX) > 0 else 0.0
...
static func _encode_resources(player: PlayerState, game_state: GameState, player_index: int, features: Array[float], offset: int) -> void:
	...
	# [0] 手牌有 Ultra Ball
	features[offset + 0] = 1.0 if _hand_has_en(player, ULTRA_BALL) else 0.0
	# [1] 手牌有 Capturing Aroma (对应 Rare Candy 位)
	features[offset + 1] = 1.0 if _hand_has_en(player, CAPTURING_AROMA) else 0.0
	# [2] 手牌有 Boss's Orders
	features[offset + 2] = 1.0 if _hand_has_en(player, BOSSS_ORDERS) else 0.0
	# [3] 手牌有 Iono
	features[offset + 3] = 1.0 if _hand_has_en(player, IONO) else 0.0
	# [4] 手牌双重无色能量数 (DTE)
	var dte_count: int = _count_energy_type_in_hand(player, "CC")
	features[offset + 4] = clampf(float(dte_count) / 3.0, 0.0, 1.0)
	# [5] 手牌草能量数
	var grass_hand: int = _count_energy_in_hand(player, "G")
	features[offset + 5] = clampf(float(grass_hand) / 4.0, 0.0, 1.0)
	# [6] 手牌超能量数
	var psychic_hand: int = _count_energy_in_hand(player, "P")
	features[offset + 6] = clampf(float(psychic_hand) / 4.0, 0.0, 1.0)
	# [7] 牌库基础能量数
	var deck_basic_energy: int = _count_basic_energy_in_deck(player)
	features[offset + 7] = clampf(float(deck_basic_energy) / 15.0, 0.0, 1.0)
	# [8] 场上总能量数
	var field_energy: int = _count_field_energy(player)
	features[offset + 8] = clampf(float(field_energy) / 10.0, 0.0, 1.0)
	# [9] Arceus VSTAR 身上能量数
	var arceus_energy: int = _count_energy_on_name(player, ARCEUS_VSTAR)
	features[offset + 9] = clampf(float(arceus_energy) / 4.0, 0.0, 1.0)
	# [10] Giratina VSTAR 身上能量数
	var giratina_energy: int = _count_energy_on_name(player, GIRATINA_VSTAR)
	features[offset + 10] = clampf(float(giratina_energy) / 4.0, 0.0, 1.0)
	# [11] 空板位数
	features[offset + 11] = clampf(float(maxi(0, 5 - player.bench.size())) / 5.0, 0.0, 1.0)
	# [12] 弃牌堆有 V/VSTAR
	features[offset + 12] = 1.0 if _has_in_discard_names_en(game_state, player_index, [ARCEUS_V, ARCEUS_VSTAR, GIRATINA_V, GIRATINA_VSTAR]) else 0.0
	# [13] 手牌有进化棋子
	features[offset + 13] = 1.0 if _hand_has_en(player, ARCEUS_VSTAR) or _hand_has_en(player, GIRATINA_VSTAR) or _hand_has_en(player, BIBAREL) else 0.0
	# [14] 手牌有 Jet Energy
	features[offset + 14] = 1.0 if _hand_has_en(player, JET_ENERGY) else 0.0
	# [15] 场上有 Maximum Belt
	var has_belt: bool = false
	var all_slots: Array[PokemonSlot] = _get_all_slots(player)
	for slot: PokemonSlot in all_slots:
		if slot.attached_tool != null:
			var tool_cd: CardData = slot.attached_tool.card_data if slot.attached_tool != null else null
			if tool_cd != null and tool_cd.name_en == MAXIMUM_BELT:
				has_belt = true
				break
	features[offset + 15] = 1.0 if has_belt else 0.0
...
static func _encode_attacker_detail(my_player: PlayerState, opp_player: PlayerState, game_state: GameState, features: Array[float], offset: int) -> void:
	## 攻击手细节 8 维
	...
	# [4] Arceus VSTAR 攻击能量缺口
	var arceus_gap: int = _attack_energy_gap(my_player, ARCEUS_VSTAR)
	features[offset + 4] = clampf(float(arceus_gap) / 4.0, 0.0, 1.0)
	# [5] Giratina VSTAR 攻击能量缺口
	var giratina_gap: int = _attack_energy_gap(my_player, GIRATINA_VSTAR)
	features[offset + 5] = clampf(float(giratina_gap) / 4.0, 0.0, 1.0)
	# [6] 对手前场剩余 HP
	features[offset + 6] = clampf(float(opp_active_hp) / 300.0, 0.0, 1.0) if opp_active_hp < 999 else 0.0
	# [7] 场上有 Lost City
	features[offset + 7] = 0.0
	if game_state.stadium_card != null:
		var stad_cd: CardData = game_state.stadium_card.card_data if game_state.stadium_card != null else null
		if stad_cd != null and stad_cd.name_en == "Lost City":
			features[offset + 7] = 1.0
...
static func _count_energy_type_in_hand(player: PlayerState, etype: String) -> int:
	## 用于匹配双重无色等特殊能量类型
	var count: int = 0
	for card: CardInstance in player.hand:
		if card != null and card.card_data != null and card.card_data.is_energy() and str(card.card_data.energy_provides) == etype:
...
static func _count_basic_energy_in_deck(player: PlayerState) -> int:
	var count: int = 0
	for card: CardInstance in player.deck:
		if card != null and card.card_data != null and card.card_data.is_energy() and card.card_data.card_type == "Energy":
			count += 1
	return count
...
static func _count_energy_on_name(player: PlayerState, name_en: String) -> int:
	var all_slots: Array[PokemonSlot] = _get_all_slots(player)
	for slot: PokemonSlot in all_slots:
		var cd: CardData = slot.get_card_data()
		if cd != null and cd.name_en == name_en:
			return slot.attached_energy.size()
	return 0
