class_name MiraidonSelfPlayDataExporter
extends RefCounted
...
var base_dir: String = "user://training_data/miraidon"
...
func record_state(game_state: GameState, current_player: int) -> void:
	var features: Array[float] = MiraidonStateEncoderScript.encode(game_state, current_player)
...
func export_game() -> String:
	...
	var payload := {
		"version": "3.0",
		"encoder": "miraidon",
		"feature_dim": MiraidonStateEncoderScript.FEATURE_DIM,
	...
	if file == null:
		push_warning("[MiraidonSelfPlayDataExporter] 无法写入: %s" % path)
		return ""
