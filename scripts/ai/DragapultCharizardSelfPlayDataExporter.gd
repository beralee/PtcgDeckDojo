class_name DragapultCharizardSelfPlayDataExporter
extends RefCounted

## 多龙巴鲁托/喷火龙卡组自博弈数据收集器。
## 使用 DragapultCharizardStateEncoder 编码特征，额外记录 teacher_score（手写 evaluate_board 归一化值）。
##
## 训练策略：
## - 只保留赢家视角的记录（输家数据丢弃）
...
var base_dir: String = "user://training_data/dragapult_charizard"
...
func record_state(game_state: GameState, current_player: int) -> void:
	var features: Array[float] = DragapultCharizardStateEncoderScript.encode(game_state, current_player)
...
func export_game() -> String:
	...
	var data := {
		"version": "3.0",
		"encoder": "dragapult_charizard",
		"feature_dim": DragapultCharizardStateEncoderScript.FEATURE_DIM,
	...
	if file == null:
		push_warning("[DragapultCharizardSelfPlayDataExporter] 无法写入: %s" % path)
		return ""
